...
class TransaksiFragment : Fragment() {
    ...
    private fun observeData() {
        ...
        viewModel.totalPendapatan.observe(viewLifecycleOwner) { total ->
            binding.tvTotalPendapatan.text =
                "Total Pendapatan: ${CurrencyFormatter.format(total ?: 0)}"
        }
        ...
        viewModel.exportFile.observe(viewLifecycleOwner) { file ->
            val intent = FileHelper.shareFile(requireContext(), file, "text/csv")
            startActivity(
                android.content.Intent.createChooser(
                    intent,
                    "Bagikan CSV Transaksi"
                )
            )
    ...
    private fun showPilihBarangDialog() {
        ...
        val namaList = barangList.map {
            "${it.nama} (Stok: ${it.jumlah} ${it.satuan})"
        }.toTypedArray()
    ...
    private fun showInputQtyDialog(barang: Barang) {
        ...
        val etQty = dialogView.findViewById<com.google.android.material.textfield.TextInputEditText>(
            R.id.etQty
        )
