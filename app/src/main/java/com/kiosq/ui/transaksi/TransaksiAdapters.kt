...
class CartAdapter(
    ...
    companion object CartDiff : DiffUtil.ItemCallback<CartItem>() {
        override fun areItemsTheSame(a: CartItem, b: CartItem) = a.barang.id == b.barang.id
    ...
    inner class VH(private val binding: ItemCartBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: CartItem) {
            ...
            binding.btnHapus.setOnClickListener { onRemove(item) }
        }
    }
...
// ──────────────── TRANSAKSI ADAPTER ───────────────────────────────────────────
class TransaksiAdapter : ListAdapter<Transaksi, TransaksiAdapter.VH>(TransaksiDiff) {

    companion object TransaksiDiff : DiffUtil.ItemCallback<Transaksi>() {
        override fun areItemsTheSame(a: Transaksi, b: Transaksi) = a.id == b.id
        override fun areContentsTheSame(a: Transaksi, b: Transaksi) = a == b
    }
    ...
    inner class VH(private val binding: ItemTransaksiBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(t: Transaksi) {
            ...
            binding.tvCatatan.text = if (t.catatan.isNotBlank()) t.catatan else "-"

            val color = when (t.jenis) {
                JenisTransaksi.JUAL.name -> Color.parseColor("#388E3C")
                JenisTransaksi.BELI.name -> Color.parseColor("#1565C0")
                else -> Color.parseColor("#F57F17")
            }
            binding.tvJenis.setTextColor(color)
            binding.chipJenis.apply {
                text = t.jenis
                setChipBackgroundColorResource(
                    when (t.jenis) {
                        JenisTransaksi.JUAL.name -> com.kiosq.R.color.green_100
                        JenisTransaksi.BELI.name -> com.kiosq.R.color.blue_100
                        else -> com.kiosq.R.color.orange_100
                    }
                )
            }
        }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) =
        VH(ItemTransaksiBinding.inflate(LayoutInflater.from(parent.context), parent, false))
